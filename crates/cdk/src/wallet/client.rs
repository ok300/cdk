...
impl HttpClientMethods for HttpClient {
    ...
    fn get_mint_keys(&self, mint_url: MintUrl) -> Result<Vec<KeySet>, Error> {
        let url = mint_url.join_paths(&["v1", "keys"])?;
        let keys = self.inner.get(url).send()?.json::<Value>()?;
    ...
    #[instrument(skip(self), fields(mint_url = %mint_url))]
    fn get_mint_keyset(&self, mint_url: MintUrl, keyset_id: Id) -> Result<KeySet, Error> {
    ...
    /// Get Keysets [NUT-02]
    #[instrument(skip(self), fields(mint_url = %mint_url))]
    fn get_mint_keysets(&self, mint_url: MintUrl) -> Result<KeysetResponse, Error> {
        let url = mint_url.join_paths(&["v1", "keysets"])?;
        let res = self.inner.get(url).send()?.json::<Value>()?;
    ...
    #[instrument(skip(self), fields(mint_url = %mint_url))]
    fn get_mint_info(&self, mint_url: MintUrl) -> Result<MintInfo, Error> {
...
pub trait HttpClientMethods: Debug {
    ...
    /// Split Token [NUT-06]
    fn post_swap(&self, mint_url: MintUrl, request: SwapRequest) -> Result<SwapResponse, Error>;
